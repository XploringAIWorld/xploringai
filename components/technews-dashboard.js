...
function TechNewsDashboardComponent() {
    ...
    const handlePublish = async () => {
        try {
            if (!url || !title || !pickDate) {
                ...
                await addDoc(collection(db, 'technews'), {
                    url,
                    title,
                    datePicked: pickDate
                });
    ...
    return (
        <div className='min-h-screen'>
            <p className='text-center align-middle'>Tech News Dashboard</p>
                ...
                <div className="w-full md:max-w-md p-4 rounded-lg shadow-md ">
                    {token === MY_TOKEN ? (
                        <div className="text-center">
                            ...
                            <Spacer y={4} />
                            <Link href={`/technewsposts?token=${token}`} className='text-blue-600' >Posts</Link>
...
export default TechNewsDashboardComponent;
